...
class BoxContainer
{
	//friend std::ostream& operator<<(std::ostream&, const BoxContainer<T>& operand); // Liner Error
	//friend std::ostream& operator<<<>(std::ostream&, const BoxContainer<T>& operand); // Works!
	//friend std::ostream& operator<<<T>(std::ostream&, const BoxContainer<T>& operand); // Another version - works as well
...
public:
	...
	// We can also put stream insertion into class
	template <typename T>
