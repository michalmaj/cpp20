...
std::optional<size_t> find_character_v3(const std::string& str, std::optional<char> c = std::nullopt)
{
	...
	// We can use better syntax for resolve our problem
...
int main()
{
	...
	if (result.has_value())
		...
		std::cout << "Didn't find our character at index" << std::endl;
	...
	if (result1.has_value())
		std::cout << "Found our character at index: " << result1.value() << std::endl;
	else
		std::cout << "Didn't find our character at index" << std::endl;
