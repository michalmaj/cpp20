...
void BoxContainer::expand(size_t new_capacity)
{
	...
	// Copy items over from old array to new
...
void BoxContainer::add(const value_type& item)
{
	if (m_size == m_capacity)
		expand(m_size + EXPAND_STEPS); // Expand in increments of 5 to optimize on the calls
	m_items[m_size] = item;
	++m_size;
}
...
void BoxContainer::operator=(const BoxContainer& source)
{
	...
	// It the capacities are different, set up a new internal array
	// that matches source, because we want object we are assigning to
	// to mach source as much as possible
